...
int main(void) {
    ...
    for (int i=0; i < input_text_length; i++){
        if ((48 <= input_text[i]) && (input_text[i] <= 57)){datacode_data_conv[i] = input_text[i] - 48;} // 数字
        if ((65 <= input_text[i]) && (input_text[i] <= 90)){datacode_data_conv[i] = input_text[i] - 55;} // 大文字
        if ((97 <= input_text[i]) && (input_text[i] <= 126)){datacode_data_conv[i] = input_text[i] - 87;} // 小文字
        if (input_text[i] == 32){datacode_data_conv[i] = 36;} // space
        if (input_text[i] == 36){datacode_data_conv[i] = 37;} // $
        if (input_text[i] == 37){datacode_data_conv[i] = 38;} // %
        if (input_text[i] == 42){datacode_data_conv[i] = 39;} // *
        if (input_text[i] == 43){datacode_data_conv[i] = 40;} // +
        if (input_text[i] == 45){datacode_data_conv[i] = 41;} // -
        if (input_text[i] == 46){datacode_data_conv[i] = 42;} // .
        if (input_text[i] == 47){datacode_data_conv[i] = 43;} // /
        if (input_text[i] == 58){datacode_data_conv[i] = 44;} // :
    }
    ...
    for (int i=0; i < (input_text_length / 2); i++){
        datacode_data_dec[i] = datacode_data_conv[i * 2] * 45 + datacode_data_conv[(i * 2) + 1] ; 
        itoa(datacode_data_dec[i], datacode_data_bin, 2); // 文字数の2進数表記を計算
        // 11bitに調整
    ...
    if (input_text_length % 2 == 1){
        datacode_data_dec[input_text_length / 2] = datacode_data_conv[input_text_length - 1];
        itoa(datacode_data_dec[input_text_length / 2], datacode_data_bin, 2); // 文字数の2進数表記を計算
        // 6bitに調整
        ...
        printf("%d: %s \n", datacode_data_dec[input_text_length / 2], datacode_data_bin);
    }
    ...
    // 以下の0-1文字列を順に一列にする
        ...
        // 数値化した文字列　datacode_data_bin 11bitだったり6bitだったり　そういえば保存してないや
    
    // 1列にした0-1文字列を8bitごとに区切る
    // 最後が8bitにならなかったら0埋め

    // 8bitの個数がデータコード数に満たなければ11101100 および 00010001を交互に付加
    // 10進数表記に変換　これが多項式の項になる
    ...
    // 多項式f(x)とg(x)を用意　 int fx[MAX_DEGREE + 1] = {0, 0,　... , 0}でいいかな
    // fx[i] でf(x)のx^iの項がわかるイメージで。
    // MAX_DEGREE は(データコード数 - 1) * 誤り訂正コード語数

    // f(x)には先ほどの10進数表記にしたデータを***ガロア体の指数表現に変換して*** 最大次数からいれていく Galois_exp[i]使えばok　残りは0埋め
    // g(x)は生成多項式　大きさ固定ならハードコーディングしていいや　https://www.swetake.com/qrcode/qr_table3.html
        // int gx[26+1] = {70, 218, 145, 153, 227, 48, 102, 13, 142, 245, 21, 161, 53, 165, 28, 111, 201, 145, 17, 118, 182, 103, 2, 158, 125, 173, 1}
